...
export function TableFilters<T>({ filters, values, onChange }: {
  filters: readonly TableFilter<T>[]; values: Record<string, string>; onChange: (id: string, value: string) => void;
}) {
  return <>{filters.map((filter) => <TextField key={filter.id} select size="small" label={filter.label}
    value={values[filter.id] ?? ''} disabled={filter.disabled} sx={{ minWidth: 200, flex: '1 1 200px' }}
