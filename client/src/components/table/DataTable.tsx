  ...
  return (
    <Box>
      <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap", mb: 2 }}>
        ...
        />
        <Button disabled={!state.hasSettings} onClick={state.reset}>
          Сбросить
        </Button>
      </Box>
      {loading ? (
        <Box role="status" sx={{ p: 4, textAlign: "center" }}>
          <CircularProgress aria-label="Загрузка данных" />
        </Box>
      ) : error ? (
        <Alert
          severity="error"
        ...
        <>
          <Typography variant="body2" role="status" sx={{ mb: 1 }}>
            Показано: {state.visibleRows.length} из {config.rows.length}
          </Typography>
